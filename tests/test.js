...
function des_ciph_test(cipher, key, vec, m1hax) {
    ...
    for (i = 0; i<vec.length; i++) {
        cipher.init(key, UCrypt.cipher.MODE_ENCRYPT, vec[i][2]);
        ct = cipher.finalize(vec[i][0]);
        ct = UCrypt.utils.byteArrayToHexStr(ct);
    ...
    for (i = 0; i<vec.length; i++) {
        cipher.init(key, UCrypt.cipher.MODE_ENCRYPT, vec[i][2]);
        ct = [];
        pt = UCrypt.utils.anyToByteArray(vec[i][0]);
        while (pt.length>5) {
            blk = pt.slice(0,5);
            pt = pt.slice(5);
    ...
    for (i = 0; i<vec.length; i++) {
        cipher.init(key, UCrypt.cipher.MODE_DECRYPT, vec[i][2]);
        pt = cipher.finalize(vec[i][1]);
        pt = UCrypt.utils.byteArrayToHexStr(pt);
        pt0 = vec[i][0];
        if (m1hax) {
            pt0 = UCrypt.utils.anyToByteArray(pt0);
            while ((pt0.length % 8)!=0) {
                pt0.push(0);
            }
            pt0 = UCrypt.utils.byteArrayToHexStr(pt0);
        } 
        if (pt0.equals(pt)) {
            print("      "+i+" : OK");
        } else {
    ...
    for (i = 0; i<vec.length; i++) {
        cipher.init(key, UCrypt.cipher.MODE_DECRYPT, vec[i][2]);
        pt = [];
        ct = UCrypt.utils.anyToByteArray(vec[i][1]);
        while (pt.length>5) {
            blk = ct.slice(0,5);
            ct = ct.slice(5);
...
if (test_ciph_des)  {
    ...
    print("---                      DES/ECB/NOPAD                 ---*/");
    hasFailure = false;
    var key_simple_ecb_nopad = new UCrypt.key.DESKey("466431296486ed9c");
    ...
    var key_triple2_ecb_nopad = new UCrypt.key.DESKey("46911e963f3af1b59d58e36af67fc7b7");
    ...
    var key_triple3_ecb_nopad = new UCrypt.key.DESKey("d044c68cc84e5c60063c8934d06e2158ff87b44e688063aa");
    ...
    cipher = new UCrypt.cipher.DES(UCrypt.padder.None, UCrypt.cipher.MODE_ECB);
    ...
    hasFailure = false;
    var key_simple_ecb_9797m1 = new UCrypt.key.DESKey("466431296486ed9c");
    ...
    var key_triple2_ecb_9797m1 = new UCrypt.key.DESKey("227ad947c4cf8d00f4bbb8b811b03a2f");
    ...
    var key_triple3_ecb_9797m1 = new UCrypt.key.DESKey("3874b13c8ce7ecc1cb40ac99e3fc8e4a4da6fa4816c118e6");
    ...
    cipher = new UCrypt.cipher.DES(UCrypt.padder.ISO9797M1, UCrypt.cipher.MODE_ECB);
    ...
    hasFailure = false;
    var key_simple_ecb_9797m2 = new UCrypt.key.DESKey("466431296486ed9c");
    ...
    var key_triple2_ecb_9797m2 = new UCrypt.key.DESKey("7876e970a2e834193e9b9c858f067ad6");
    ...
    var key_triple3_ecb_9797m2 = new UCrypt.key.DESKey("cba05370f80e17bb8d5c57de3bef602546779e01ea18ef5a");
    ...
    cipher = new UCrypt.cipher.DES(UCrypt.padder.ISO9797M2, UCrypt.cipher.MODE_ECB);
    ...
    print("---                      DES/CBC/NOPAD                 ---*/");
    hasFailure = false;
    var key_simple_cbc_nopad = new UCrypt.key.DESKey("466431296486ed9c");
    ...
    var key_triple2_cbc_nopad = new UCrypt.key.DESKey("e634b2877cd10a3702fd03ee68191e0b");
    ...
    var key_triple3_cbc_nopad = new UCrypt.key.DESKey("a5be277f300a0e3891669a9b3472c9e934588dd4d4e63c44");
...
function des_sign_test(signature, key, vec) {
    ...
    print("    sign/finalize");
    for (i = 0; i<vec.length; i++) {
        signature.init(key, UCrypt.signature.MODE_SIGN, vec[i][2]);
    ...
    for (i = 0; i<vec.length; i++) {
        signature.init(key, UCrypt.signature.MODE_VERIFY, vec[i][2]);
    ...
    for (i = 0; i<vec.length; i++) {
        signature.init(key, UCrypt.signature.MODE_VERIFY, vec[i][2]);
        pt = [];
        ct = UCrypt.utils.anyToByteArray(vec[i][0]);
        while (pt.length>5) {
            blk = ct.slice(0,5);
            ct = ct.slice(5);
...
if (test_sign_des)  {
    ...
    print("---                      DES/CBC/NOPAD                 ---*/");
    hasFailure = false;
    var key_sign_simple_cbc_nopad = new UCrypt.key.DESKey("9d58e36af67fc7b7");
    ...
    var key_sign_triple2_cbc_nopad = new UCrypt.key.DESKey("24e7e961efdeecb4c8f3467a82c34a26");
    ...
    var key_sign_triple3_cbc_nopad = new UCrypt.key.DESKey("a048028e1eb63357714ea916e81579ddab64d532490a90b7");
    ...
    signature = new UCrypt.signature.DES(UCrypt.padder.None, UCrypt.signature.MODE_CBC);
...
function rsa_cipher_test(cipher, pubkey, privkey, vec, nopad) {
   ...
   //test decrypt
    print("    decrypt/finalize");
    for (i = 0; i<vec.length; i++) {
        cipher.init(privkey, UCrypt.cipher.MODE_DECRYPT);
        pt = cipher.finalize(vec[i][1]);
        pt = UCrypt.utils.byteArrayToHexStr(pt);
        pt0 = vec[i][0];        
        if (pt0.equals(pt)) {
    ...
    print("    encrypt/finalize");
    for (i = 0; i<vec.length; i++) {
        cipher.init(pubkey, UCrypt.cipher.MODE_ENCRYPT);
        ct = cipher.finalize(vec[i][0]);
        cipher.init(privkey, UCrypt.cipher.MODE_DECRYPT);
...
if (test_ciph_rsa) {
    ...
    print("====================================================================");
    // --- 1024/nopad ---
    rsaciph = new UCrypt.cipher.RSA(UCrypt.padder.None);
    rsaciphpubkey = new UCrypt.key.RSAPublicKey(1024, 
                                         "00010001",
    ...
    rsaciphprivkey = new UCrypt.key.RSAPrivateKey(1024,
    ...
    // --- 1024/pkcs1 ---
    rsaciph = new UCrypt.cipher.RSA(UCrypt.padder.PKCS1_V1_5);
    rsaciphpubkey = new UCrypt.key.RSAPublicKey(1024, 
                                         "00010001",
...
function rsa_sign_test(signature, pubkey, privkey, vec) {
   var i, ct,pt,blk;
    ...
    print("    sign/finalize");
    for (i = 0; i<vec.length; i++) {
        signature.init(privkey, UCrypt.signature.MODE_SIGN);
        ct = signature.sign(vec[i][0]);
        ct = UCrypt.utils.byteArrayToHexStr(ct);
        if (vec[i][1].equals(ct)) {
            print("      "+i+" : OK");
    ...
    print("    verify/finalize");
    for (i = 0; i<vec.length; i++) {
        signature.init(pubkey, UCrypt.signature.MODE_VERIFY);
        pt = signature.verify(vec[i][0], vec[i][1]);
        if (pt) {
            print("      "+i+" : OK");
        } else {
...
if (test_sign_rsa) {
    ...
    // --- 1024/SHA1 ---
    rsasighasher = new UCrypt.hash.SHA1();    
    rsasig = new UCrypt.signature.RSA(rsasighasher, UCrypt.padder.PKCS1_V1_5);
    rsapubkey = new UCrypt.key.RSAPublicKey(1024, 
                                         "00010001",
    ...
    rsaprivkey = new UCrypt.key.RSAPrivateKey(1024,
...
if (test_hash) {
    ...
    function test(name, hash, answers) {
        ...
        for (i = 0; i<inputs.length;i++) {
            hash.reset();
            blk = UCrypt.utils.strToByteArray(inputs[i]);
            r = hash.finalize(blk);
            r = UCrypt.utils.byteArrayToHexStr(r);
        ...
        hash.reset();
        blk = UCrypt.utils.strToByteArray(inputs[inputs.length-1]);    
        while (blk.length > 11) {
    ...
    test("SHA1",      new UCrypt.hash.SHA1(),      sha1_answers);
    test("SHA224",    new UCrypt.hash.SHA224(),    sha224_answers);
    test("SHA256",    new UCrypt.hash.SHA256(),    sha256_answers);
    test("RIPEMD160", new UCrypt.hash.RIPEMD160(), ripemd160_answers);
...
if (test_ecfp) {
    ...
    var d =  UCrypt.ECFp.getEcDomainByName("secp256k1");
    var p = new UCrypt.ECFp.AffinePoint("edc8530038d1186b9054acb75aef1419e78ae29b7ee86d42d2dc675504367421",
    ...
    u = UCrypt.utils.byteArrayToHexStr(u);
...
if (test_ka_ecdh) {
    ...
    secret = UCrypt.utils.byteArrayToHexStr(secret);
...
if (test_sign_ecdsa) {
    ...
    //secp192r1
    domain =  UCrypt.ECFp.getEcDomainByName("secp192r1");
    privkey = new UCrypt.key.EcFpPrivateKey(
        256, domain, 
        "98de9c2529ad3498287c84c5fbe94f1c1ffc0ed6e62f1b88");
    pubkey = new UCrypt.key.EcFpPublicKey(
        256, domain, 
        new UCrypt.ECFp.AffinePoint("c8a20e0b34dc0c3f96e5a435fac917042e21c9a5c24815a3",
    ...
    for (var i = 0; i<domains.length; i++) {
        domain =  UCrypt.ECFp.getEcDomainByName(domains[i]);
        [pubkey, privkey] =  UCrypt.key.generateECFpPair(256, domain);
